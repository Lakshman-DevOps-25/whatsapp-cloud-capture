...
export async function generateLongToken(shortToken) {
  ...
  // Always read from MongoDB first — guarantees latest values even if
  // process.env wasn't populated (e.g. called before loadConfigFromDB)
  const { getConfig } = await import('./configService.js');

  const appId     = process.env.WA_APP_ID     || await getConfig('WA_APP_ID');
  const appSecret = process.env.WA_APP_SECRET || await getConfig('WA_APP_SECRET');
  const token     = shortToken || process.env.WA_ACCESS_TOKEN || await getConfig('WA_ACCESS_TOKEN');

  if (!appId)     throw new Error('WA_APP_ID not found in process.env or MongoDB — seed it via POST /api/admin/config');
  if (!appSecret) throw new Error('WA_APP_SECRET not found in process.env or MongoDB — seed it via POST /api/admin/config');
  if (!token)     throw new Error('WA_ACCESS_TOKEN not found — seed it via POST /api/admin/config');

  // Ensure process.env is up to date for this request
